...
START:
	...
	mov si, 0	; initialize string source index
...
times 510 - ($ - $$)	db	0x00	; $ : current line's addr, $$ : current section's base address -> $ - $$ : Offset!
					; $10 - ( $ - $$ ) : offset to addr $10, db - 0x00 : declare 1byte and init to 0x00
					; time : loop -> fill 0x00 from current address to $10

db 0x55	; declare 1 byte and init to 0x55
db 0xAA	; declare 1byte and init to 0xAA
	; Address 511 : 0x55, 512 : 0xAA -> decalre that this sector is boot sector
